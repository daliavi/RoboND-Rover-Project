...
def easy_angle(a): return a - 360 if a > 180 else a
...
def telemetry(sid, data):
    ...
    if data:
        ...
        if np.isfinite(Rover.vel):
            ...
            roll_min, roll_max = -1.0, 1.0
            # only run perception on valid images.
            if pitch_min <= easy_angle(Rover.pitch) <= pitch_max and roll_min <= easy_angle(Rover.roll) <= roll_max:
